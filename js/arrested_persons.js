...
function getCriminalDetails(record) {
    if (record.criminals && record.criminals.length > 0) {
        const criminal = record.criminals[0];
        // Ensure imageBase64 is passed through
        return {
            name: criminal.name,
            age: criminal.age,
            address: criminal.address,
            marks: criminal.marks,
            alias: criminal.alias,
            gender: criminal.gender,
            record: criminal.record,
            history: criminal.history,
            imageBase64: criminal.imageBase64 || null   // ← FIX: use imageBase64
        };
    }
    if (record.criminalDetails) {
        // Old format: criminalDetails might have imageUrl or imageBase64
        return {
            ...
            imageBase64: record.criminalDetails.imageBase64 || record.criminalDetails.imageUrl || null, // ← support both
...
function renderCards(data) {
    ...
    data.forEach(item => {
        ...
        // FIX: use imageBase64 instead of imageUrl
        const photoBase64 = item.criminal.imageBase64;
        let photoHtml = '';
        if (photoBase64 && photoBase64.startsWith('data:image')) {
            photoHtml = `<img src="${photoBase64}" class="card-photo" onerror="this.src='https://via.placeholder.com/70?text=No+Image'">`;
        } else {
            photoHtml = `<div class="card-photo-placeholder"><i class="fas fa-user"></i></div>`;
        }
...
function showDetails(item) {
    ...
    let photoHtml = '';
    // FIX: use imageBase64
    if (criminal.imageBase64 && criminal.imageBase64.startsWith('data:image')) {
        photoHtml = `<div><img src="${criminal.imageBase64}" class="modal-photo" alt="Criminal Photo" style="max-width:200px; border-radius:10px; margin-bottom:15px;"></div>`;
    }
    modalBody.innerHTML = `
        ${photoHtml}
